...
//       if (response.success) {
//         return response[dataKey] || response.data || response;
//       } else {
...
//         processedShops = shopsData;
//       } else if (shopsData && typeof shopsData === 'object') {
//         const possibleArrays = Object.values(shopsData).filter(value => Array.isArray(value));
//         if (possibleArrays.length > 0) {
//           processedShops = possibleArrays[0];
//         }
//       }
...
//   // ✅ ENHANCED: Get shop name with multiple fallback strategies
...
//     // Priority 3: Look up shop from shops list using shop ID
//     if (product.shop) {
...
//         // ✅ ENHANCED: Ensure all products have proper shop names
//         const enhancedProducts = productsData.map(product => ({
//           ...product,
//           // Ensure shopName is properly set for display
...
//         const possibleArrays = Object.values(productsData).filter(value => Array.isArray(value));
//         if (possibleArrays.length > 0) {
//           const productsArray = possibleArrays[0];
//           // ✅ ENHANCED: Ensure all products have proper shop names
...
//   // ✅ ENHANCED: Prepare product data with proper shop information
...
//     // ✅ CRITICAL: Ensure both shop ID and shop name are included
//     const productData = {
...
//       shop: values.shop, // Shop ID for reference
//       shopName: selectedShop.name // Shop name for display
//     };

//     console.log('📤 Prepared product data with shop:', {
...
//           form.resetFields();
//         } else {
//           const errorMsg = response?.message || response?.error || 'Failed to update product';
...
//           form.resetFields();
//         } else {
//           const errorMsg = response?.message || response?.error || 'Failed to create product';
//           throw new Error(errorMsg);
//         }
//       }
...
//       } else {
//         const errorMessage = error.message || (editingProduct ? 'Failed to update product' : 'Failed to add product');
//         message.error(errorMessage);
//       }
//     } finally {
//       setLoading(false);
...
//     if (product.shop && typeof product.shop === 'object') {
//       return product.shop._id || product.shop.id;
...
//           <Tag 
//             color={getStockStatus(record).status} 
//             style={{ 
//               fontSize: '10px', 
//               marginTop: '4px',
//               border: 'none'
...
const ProductManagement = () => {
  ...
  // ✅ NEW: Shop filter options including "All Shops"
  ...
  // ✅ NEW: Category filter options including "All Categories"
  ...
  // ✅ NEW: Filter products based on search text and filters
